...
neuralNet<-function(Input,response,InputTest=NULL,respTest=NULL, #input data
                    ...
                    optAlg="GD", #can be 'Adam', 'RMSProp', 'Momentum'
                    ...
                    #BatchNorm: binary for batch normalization
  ...
  active=init(DW,Input,rsp$respMat,activationFunc,weightsVector,optAlg)
  optimize=OptimizeCost(weight=active$W,resp=rsp,X=active$In,respT=respTest,XT=InputTest, #data
                        ...
                        trW=traceWeights,Epochs=epochs,Tolerance=tol,weightsVec=active$weightsVec, #debugging
                        optimization=optAlg,beta1=beta1,beta2=beta2,momentum=active$momentum,rmsprop=active$rmsprop) 
  t2=Sys.time()
